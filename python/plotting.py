...
def common_plot_options_int():
    ...
    plt.xlim(118,128)
    plt.xticks(np.arange(118,128,2))
...
def plot_data(hy, hyerr, hx, fhx, fpars, fparerrs, chi2ndf, plotdir, mass, hname):
    ...
    plt.savefig('%s/M%d/%s.png'%(plotdir,int(mass),hname))
    plt.savefig('%s/M%d/%s.pdf'%(plotdir,int(mass),hname))
    plt.close()
